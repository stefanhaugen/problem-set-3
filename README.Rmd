...
summ_stats %>% ungroup() %>% arrange(desc(mean, var)) %>% slice(1:3) %>% select(gene.name) %>% unlist()-> top_genes
```
...
```{r problem_5}
cleaned_data2 %>% filter(gene.name %in% top_genes) -> topgeneplot
topgeneplot %>% ggplot(aes(as.numeric(time.value), value, color = gene.name, shape = rep)) + 
  geom_point() +
