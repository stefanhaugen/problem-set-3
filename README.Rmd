...
cleaned_data %>% mutate(time.point = str_replace(time.point, 'hpi', ''), time.point = str_c('hpi.', time.point))
...
cleaned_data %>% arrange(time.point, desc(value)) %>% group_by(time.point = 'hpi.6') %>% head(1)
...
cleaned_data %>% arrange(time.point, desc(value)) %>% group_by(time.point = 'hpi.18') %>% head(1) %>% select(plant, fungus)
...
cleaned_data %>%
  mutate(time.value = str_replace(time.point, 'hpi', ''),
         time.hpi = str_c('hpi.', time.value)) -> cleaned_data2

cleaned_data2
```

2. Create a new column containing the expression difference between the relevant time points.
```{r new_column, eval=FALSE}
cleaned_data2 %>% select(-time.value, -time.point) %>% spread(time.hpi, value) %>% mutate(exp_diff = hpi.24 - hpi.6) -> cleaned_expdiff
cleaned_expdiff
```
...
```{r summ statistics, eval = FALSE}
cleaned_expdiff %>% group_by(gene.name, plant, fungus) %>% summarise(mean = mean(exp_diff), var = var(exp_diff)) -> summ_stats
summ_stats
```
...
```{r 3subset4}
summ_stats %>% ungroup() %>% select(-gene.name) %>% arrange(desc(mean, var)) %>% slice(1:3)
```
...
`r cleaned_data %>% summarise(n_distinct(gene.name))` distinct genes were analyzed across `r cleaned_data %>% summarise(n_distinct(plant))` different plant species, as well as `r cleaned_data2 %>% summarise(n_distinct(fungus))` fungus variants. It was discovered that the most variably expressed were: `r summ_stats %>% ungroup() %>% select(-gene.name) %>% arrange(desc(mean, var)) %>% slice(1:3)`

##I'm not exactly sure how to use the `r ` inline text concept.. Would I keep it out of the ``` partitioned section?
...
topgeneplot %>% ggplot(aes(time.point, value, color = gene.name, shape = rep)) + geom_point()
## I'm unsure how to add a geom_smooth line between points? also the facet portion?
```
